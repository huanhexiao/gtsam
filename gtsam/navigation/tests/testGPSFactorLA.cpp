 ...
 * @brief   Unit test for GPSFactorLA
...
TEST(GPSFactorLA, Constructor) {
  ...
  GPSFactorLA factor(key, Point3(E, N, U), model);
  ...
  Matrix expectedH = numericalDerivative11<Vector, Pose3>(
      boost::bind(&GPSFactorLA::evaluateError, &factor, _1, boost::none), T);
...
// *************************************************************************
TEST(GPSFactorLA, ConstructorWithLeverArm) {
  using namespace example;
  ...
  Key key(1);
  SharedNoiseModel model = noiseModel::Isotropic::Sigma(3, 0.25);
  Point3 body_t(-0.7, 0.4, -1.2);
  GPSFactorLA factor(key, Point3(E, N, U), model, body_t);

  // Create a linearization point at zero error
  Pose3 T(Rot3::RzRyRx(0.15, -0.30, 0.45), Point3(E, N, U));

  Point3 expectedError(-0.552365503, 0.371564563, -1.28328957);
  EXPECT(assert_equal(expectedError, factor.evaluateError(T), 1e-5));
  ...
  std::cout << "expectedH:" << expectedH << endl;
  std::cout << "actualH:" << actualH << endl;
...
TEST(GPSData, init) {
  ...
  Vector3 nV;
  boost::tie(T, nV) =
      GPSFactorLA::EstimateState(t1, NED1, t2, NED2, 84831.0796);
